...
export interface OptionalFeatureExporterResult {
    count: number;
    data: Record<string, any>[];
}

export const runOptionalFeatureExporter = async (
    profiles: ExportProfile[],
    deps: { idMgr: IdMgrLike; logger: LoggerLike }
): Promise<OptionalFeatureExporterResult> => {
    const result = await runGenericFileExporter(profiles, deps);
    const profile = profiles.find(p => p.dataType === 'optionalfeature');
    return {
